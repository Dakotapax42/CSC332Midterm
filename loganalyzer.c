...
int main(int argc, char *argv[]) {
    ...
    // if filepath is not provided, print usage and exit safely
    if (!filepath) {
        printf("Usage: loganalyzer -f <filename>\n");
        return 1;
    }
    // open the file for reading only, if it fails print error and exit safely
    int fd = open(filepath, O_RDONLY);
    if (fd < 0) { perror("open"); return 1; }

    struct stat s;
    fstat(fd, &s);
    // edge case: if file is empty, print message then exit safely without trying to map it
    if (s.st_size == 0) {
        printf("File is empty.\n");
        close(fd);
        return 0;
    }
    // map the file into memory for efficient access and analysis, if it fails print error and exit safely
